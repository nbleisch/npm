...
function search (args, silent, staleness, cb) {
  ...
  updateIndex(staleness, function (er, stream) {
    if (er) return cb(er)
    log.silly('search', 'filtering search index data')
    stream.on('data', function (pkg) {
      log.gauge.pulse('search')
      if (!silent && filter(pkg, opts, searchexclude)) {
        log.silly('search', 'matched', pkg.name)
        output(pkg.name, ':', pkg.description)
      }
    })
    stream.on('error', function (pkg) {
      log.verbos('search', 'index update stream error:', er.message)
    })
    stream.resume()
