...
function createCacheEntryStream (cacheFile, cb) {
  ...
  fs.stat(cacheFile, function (err, stat) {
    ...
    var entryStream = ms.pipeline(
      fs.createReadStream(cacheFile, 'utf8'),
      jsonstream.parse('*')
    )
...
function extractUpdated (entryStream, cb) {
  ...
  entryStream.on('end', nope)
  entryStream.once('data', function (latest) {
    log.silly('update-index', 'got first cache entry: ', latest)
  ...
  getCacheStat(function (er, st) {
    ...
    mkdir(cacheBase, function (er, made) {
      ...
      chownr(made || cacheBase, st.uid, st.gid, function (er) {
        ...
        return cb(null, ms.through.obj(function (pkg, enc, cb) {
          if (!updatedWritten) {
          ...
          writeStreamAtomic(cacheFile)
        ).pipe(
          // We want this stream to be passthrough, so we have to
          // reparse :(
          jsonstream.parse('*')
        ))
      })
