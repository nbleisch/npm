...
function createCacheWriteStream (cacheFile, latest, cb) {
  ensureCacheDirExists(cacheFile, function (err) {
    if (err) return cb(err)
    log.silly('update-index', 'creating output stream')
    var outStream = createCacheOutStream()
    var cacheFileStream = writeStreamAtomic(cacheFile)
    var inputStream = createCacheInStream(cacheFileStream, outStream, latest)
...
function createCacheInStream (writer, outStream, latest) {
  var updatedWritten = false
  return ms.pipeline.obj(
    ms.through.obj(function (pkg, enc, cb) {
      if (!updatedWritten) {
        // _updated is an internal implementation detail, but the cache
        // expects it as the first entry in the file. We sneak it in here
        // by hand, even though it's passed in as an argument.
...
function ensureCacheDirExists (cacheFile, cb) {
