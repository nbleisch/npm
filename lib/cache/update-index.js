...
function extractUpdated (entryStream, label, cb) {
  ...
  // TODO - for some reason, when the stream ends right away, this isn't
  //        picking it up, and we get a `cb() not called` error.
  //        This happens mainly with empty requests, or empty-but-present local
  //        index caches.
  ...
  getCacheStat(function (er, st) {
    ...
    mkdir(cacheBase, function (er, made) {
      ...
      chownr(made || cacheBase, st.uid, st.gid, function (er) {
        ...
        // `writeStreamAtomic` streams aren't passthrough, so we have
        // to do a bit of a dance if we want the stream returned from this
        // function to *act* as a passthrough, and end correctly when the
        // write stream ends or errors, etc.
        var writer = writeStreamAtomic(cacheFile)
        var passThrough = ms.pipeline.obj(
          ms.through.obj(function (pkg, enc, cb) {
            if (!updatedWritten) {
              // We store `latest` as an entry partly for historical
              // reasons, partly out of convenience.
          ...
          ms.through(function (chunk, enc, cb) {
            // This is what simulates the passthrough: chunks do not get
            // written through to the next step of this stream until
            // the file stream is finished with them.
            writer.write(chunk, enc, function () {
              cb(null, chunk)
            })
          }),
          // Since we want the stream to wait until after each bit of data
          // is written, we reparse what we just wrote, just to keep things
          // orderly.
          jsonstream.parse('*')
        )
