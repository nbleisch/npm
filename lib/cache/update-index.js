...
function updateIndex (staleness, args, notArgs, filter, cb) {
  ...
  mapToRegistry('-/all', npm.config, function (er, uri, auth) {
    ...
    getCacheFiles(cacheBase, function (err, cacheFiles) {
      if (err) return cb(err)
      collectCachedMetadata(cacheBase, cacheFiles, filter, args, notArgs, function (err, cachedData, latest) {
        if (err) return cb(err)

        // use the cache and make no request if it's not too old
        if (Date.now() - latest < MAX_SEARCH_CACHE_AGE) {
          finish(cachedData, cb)
        } else {
          if (latest === 0) {
...
function getCacheFiles (cacheBase, cb) {
...
function collectCachedMetadata (cacheBase, cacheFiles, filter, args, notArgs, cb) {
  var latest = 0
